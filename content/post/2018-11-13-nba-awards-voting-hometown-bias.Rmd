...
kable(lawler.ex, digits = 2) %>%
  kable_styling(bootstrap_options = c("striped", "bordered", "condensed"),
                full_width = FALSE) %>%
  row_spec(lawler.clips, bold = TRUE, color = lawler.cols$hex2, 
           background = lawler.cols$hex1)
...
homer.sort <- final.summary.z %>%
  select(Voter, Team.Aff, Role, n, avg, n.pre, avg.pre, n.post, avg.post) %>%
  arrange(-avg)
...
datatable(homer.sort, class = "cell-border stripe", options = list(dom = "lftp")) %>%
...
# htmltools::div(hide_legend(team.plot), align = "center")
...
ind.ballot.fun <- function(cur.voter, cur.team, cur.rank) {
  ...
  out1 <- out1 %>%
    ...
    kable_styling(bootstrap_options = c("striped", "bordered", "condensed"), 
                  full_width = FALSE) %>%
    row_spec(bal.team, bold = TRUE, color = bal.col$hex2, 
             background = bal.col$hex1)
  ...
  out2 <- out2 %>%
    ...
    kable_styling(bootstrap_options = c("striped", "bordered", "condensed"), 
                  full_width = FALSE) %>%
    row_spec(result.team, bold = TRUE, color = bal.col$hex2, 
             background = bal.col$hex1)
